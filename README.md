...
```bash
# 基本用法
txt2epub -f your-novel.txt -a "作者名" -t "书名"

# 完整参数
txt2epub --file your-novel.txt --author "作者名" --title "书名" --cover cover.jpg --output output.epub
...
| `--description`   | `-d` | ❌   | 书籍描述             | `-d "一部精彩小说"` |
| `--no-ads-filter` |      | ❌   | 禁用广告过滤         | `--no-ads-filter`   |
...
可以通过 `--no-ads-filter` 参数或 API 选项禁用。
