...
class TxtToEpubConverter {
  ...
  processTextFile(filePath) {
    ...
    if (this.options.filterAds) {
      ...
      const filteredLines = lines.filter((line) => {
        return !this.options.adKeywords.some((keyword) => (line + "").toLocaleLowerCase().includes((keyword + "").toLocaleLowerCase()));
      });
  ...
  async convertTxtToEpub(options) {
    const { inputFile, outputFile, title, author, maker, adKeywords, cover, description } = options;
