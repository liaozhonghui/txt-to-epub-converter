...
class TxtToEpubConverter {
     ...
     */
    generateCopyrightPage(title, author, totalChapters, totalWords) {
    ...
    async convertTxtToEpub(options) {
        ...
        if (!inputFile || !outputFile || !title || !author) {
            throw new Error('缺少必要参数: inputFile, outputFile, title, author');
        }
        ...
        try {
            ...
            const epubChapters = [
                ...
                {
                    title: '图书信息',
                    data: this.generateCopyrightPage(title, author, chapters.length, totalWords)
                },
                // 正文章节
